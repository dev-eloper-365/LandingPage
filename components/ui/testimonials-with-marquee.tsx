        ...
        <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
          <div className="group flex overflow-hidden p-2 [--gap:1rem] [gap:var(--gap)] flex-row [--duration:80s]">
            <div className="flex shrink-0 justify-around [gap:var(--gap)] animate-marquee flex-row group-hover:[animation-play-state:paused]">
              {testimonials.map((testimonial, i) => <TestimonialCard key={`original-${i}`} {...testimonial} />)}
              {testimonials.map((testimonial, i) => <TestimonialCard key={`duplicate-1-${i}`} {...testimonial} />)}
              {testimonials.map((testimonial, i) => <TestimonialCard key={`duplicate-2-${i}`} {...testimonial} />)}
              {testimonials.map((testimonial, i) => <TestimonialCard key={`duplicate-3-${i}`} {...testimonial} />)}
              {testimonials.map((testimonial, i) => <TestimonialCard key={`duplicate-4-${i}`} {...testimonial} />)}
              {testimonials.map((testimonial, i) => <TestimonialCard key={`duplicate-5-${i}`} {...testimonial} />)}
            </div>
          </div>
